...
Modified by Pamela Carreno for inclusion of a visualizer within Jupiter notebook
"""
...
class RobotJupyterPlot(object):
    ...
    def __init__(
        ...
        # Text to display end effector transform
        self.ee_text = None
    ...
    def init(self):
        ...
        # Plot the shadow of the robot links, draw first so robot is always in front
        if self.shadow:
            self.sh_links = self.ax.plot(
                loc[0, :], loc[1, :], np.zeros(loc.shape[1]),
